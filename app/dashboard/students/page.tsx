...
const tabChangeAnimation = `
  ...
  .tab-change-animation {
    animation: tabPulse 0.3s ease-in-out;
  }
`
...
export default function StudentsPage() {
  ...
  const handleTabChange = (event: React.SyntheticEvent, newValue: number) => {
    ...
    // Reset search term when changing tabs for better UX
    setSearchTerm("")
  }
  ...
  const getFilteredStudents = () => {
    return students
      .filter((student) => {
        // Filtro por status baseado na aba selecionada
        if (tabValue === 0) return true // Todos
        if (tabValue === 1) return student.status === "active" // Ativos
        if (tabValue === 2) return student.status === "pending" // Pendentes
        if (tabValue === 3) return student.status === "inactive" // Inativos
        return true
      })
      .filter((student) => {
  ...
  const getTabColor = (index: number) => {
    if (tabValue === index) return "#4ade80"
    return "#d1d5db"
  }
  ...
  useEffect(() => {
    // Efeito visual ao mudar de aba
    const tabIndicator = document.querySelector(".MuiTabs-indicator")
    if (tabIndicator) {
      tabIndicator.classList.add("tab-change-animation")
      setTimeout(() => {
        tabIndicator?.classList.remove("tab-change-animation")
      }, 300)
    }
  }, [tabValue])
  ...
  return (
    <Box sx={{ p: 3 }}>
      <style jsx global>{`${tabChangeAnimation}`}</style>
      {/* Header */}
      ...
      >
        ...
        >
          <Tab label="Todos" sx={{ color: getTabColor(0) }} />
          <Tab label="Ativos" sx={{ color: getTabColor(1) }} />
          <Tab label="Pendentes" sx={{ color: getTabColor(2) }} />
          <Tab label="Inativos" sx={{ color: getTabColor(3) }} />
        </Tabs>
      </Paper>
