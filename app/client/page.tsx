      ...
      >
        <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 rounded-3xl p-6 border border-green-500/30">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-white mb-2">Olá, {clientData.name}! 👋</h1>
              <p className="text-gray-300">
                Seu personal trainer <span className="text-green-400 font-medium">{clientData.trainer}</span> preparou
          ...
          <div className="bg-black border border-gray-800 rounded-3xl p-6">
            ...
            <div className="bg-gradient-to-r from-green-500/10 to-blue-500/10 rounded-2xl p-6 border border-green-500/20">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-lg font-bold text-white mb-2">{clientData.nextWorkout.name}</h3>
                  <div className="flex items-center gap-4 text-sm text-gray-300">
                    <span className="flex items-center gap-1">
                      <Clock className="w-4 h-4" />
                      {clientData.nextWorkout.duration}
                    </span>
                    <span className="flex items-center gap-1">
                      <Dumbbell className="w-4 h-4" />
                      {clientData.nextWorkout.exercises} exercícios
                    </span>
                    <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded-lg text-xs">
                      {clientData.nextWorkout.difficulty}
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-green-400 font-medium text-sm">{clientData.nextWorkout.scheduled}</div>
                </div>
              </div>
        ...
        >
          <div className="bg-black border border-gray-800 rounded-3xl p-6">
            ...
            <div className="space-y-4">
              {clientData.weeklyGoals.map((goal, index) => {
