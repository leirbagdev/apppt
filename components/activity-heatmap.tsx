...
export default function ActivityHeatmap() {
  ...
  // Função para gerar dados de exemplo para o heatmap
  const generateHeatmapData = () => {
    const data = []

    // Primeiras 3 semanas com dados específicos
    const specificWeeks = [
      [
    ...
    // Adicionar as semanas específicas
    data.push(...specificWeeks)
  ...
  return (
    <div className="overflow-x-auto">
      <div className="inline-block min-w-full">
        <div className="pl-8 flex gap-1 mb-1">
          {months.map((month, index) => (
            <div key={index} className="text-xs text-zinc-400 flex-basis-[calc(4*(16px+4px))] min-w-[55px]">
              {month}
            </div>
          ))}
        </div>
        ...
        <div className="flex">
          <div className="flex flex-col justify-around h-[calc(7*(16px+4px)-4px)] pr-2 text-xs text-zinc-400 w-8">
                  ...
                  <TooltipProvider key={dayIndex}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div
                          className={`w-4 h-4 rounded-sm border border-zinc-800/20 hover:scale-110 hover:outline hover:outline-2 hover:outline-emerald-500/50 hover:outline-offset-1 hover:z-10 transition-transform ${getLevelClass(day.level)}`}
                        ></div>
                      </TooltipTrigger>
