  ...
  return (
    <div
      className={cn(
        ...
        {
          "border border-[var(--outline)]": outlined,
          "bg-[var(--surface-variant)]": filled,
          "w-full": fullWidth,
...
export function CardHeader({ className, title, subtitle, action, avatar, compact = false, ...props }: CardHeaderProps) {
  return (
    <div className={cn("flex items-start gap-4 p-6 text-[var(--on-surface)]", compact && "p-4", className)} {...props}>
      {avatar && <div className="flex-shrink-0">{avatar}</div>}
      <div className="flex-1 min-w-0">
        <h3 className={cn("font-medium text-lg leading-none", compact && "text-base")}>{title}</h3>
        {subtitle && <p className={cn("text-[var(--on-surface-variant)] mt-1", compact && "text-sm")}>{subtitle}</p>}
      </div>
